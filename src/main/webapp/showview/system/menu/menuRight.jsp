    <%@ page language="java" contentType="text/html; charset=UTF-8"
        ...
        ,url:'${xq2580z}/menu/loadAllMenu' //数据接口
        ...
        alert(params);
        tableIns.reload({
        url:"${xq2580z}/menu/loadAllMenu?"+params
        })
        });
        ...
        if(layEvent === 'del'){ //删除
        //先判断当前菜单有没有子节点
        $.post("${xq2580z}/menu/checkMenuHasChildren?id="+data.id,function(obj){
        if(obj.code>=0){
        layer.msg("当前菜单有子节点同，请先删子节点");
        }else{
        layer.confirm('真的删除【'+data.title+'】这个菜单吗', function(index){
        //向服务端发送删除指令
        $.post("${xq2580z}/menu/deleteMenu",{id:data.id},function(res){
        layer.msg(res.msg);
        //刷新数据 表格
        tableIns.reload();
        //刷新左边的树
        ...
        url: "${xq2580z}/menu/loadMenuManagerLeftTreeJson?spread=1", // 使用url加载（可与data加载同时存在）
        icon: "2",
        accordion:true
        });
        ...
        tableIns.reload({
        url:"${xq2580z}/menu/loadAllMenu?id="+id
        })
        }
        </script>
        </body>
