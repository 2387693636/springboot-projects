package com.xq.statistics.utils;
 ...
 * @description: statistics统计客户数据库导出
 */
		...
		try {
			...
			if(null != codeNormal) {
				if(null != logoStream) {
					...
					// 计算应画在二维码上的宽高
					int code_logo_width = Math.min(((code_width) / 5), image_width);
					int code_logo_height = Math.min(((code_height) / 5), image_height);
